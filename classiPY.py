...
def label_folder(input_folder, output_folder, classification='CUI'):
    ...
    image_files = [x for x in Path(input_folder).iterdir()
                   if x.suffix.lower() in extensions]
    
    for image_file in image_files:
        output_file = Path(output_folder,
                           f"({markings.get_symbol(classification)}) \
                           {image_file.name}")
...
def main():
    ...
    required_group.add_argument('--classification', '-C', required=True, type=str, choices=markings.classifications,
