...
def add_banner_and_border_to_image(input_image, output_image):
    with Image(filename=input_image) as img:
        ...
        with Drawing() as draw:
            ...
            draw.text(x=banner.width // 2, y=banner_height // 2 + font_size // 3, body='SECRET')
            draw.text(x=banner.width // 2, y=img.height + banner_height + banner_height // 2 + font_size // 3, body='SECRET')
            draw(banner)
